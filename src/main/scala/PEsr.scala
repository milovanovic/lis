...
class PEsr [T <: Data: Real] (val params: LISParams[T], index: Int) extends Module {
  ...
  if (index < params.LISsize + 1) {
    val compare_1_sel = Mux(io.sortDir.getOrElse(params.sortDir.B), io.sorted_data1.get < io.data_rm.get, io.sorted_data1.get > io.data_rm.get)
    ...
    val mux_2 = if (index == 1) Mux(compare_2, mux_1, io.data_insert) else Mux(run_time_mux_1, mux_1, Mux(run_time_mux_0, io.data_insert, io.prev_data.get))
    dontTouch(mux_2)
...
// comment: suggestName and dontTouch are only there for easier debugging and testing
