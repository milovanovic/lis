...
class LinearSorterSR[T <: Data: Real](val params: LISParams[T]) extends Module {
  ...
  if (params.LISsubType == "LIS_FIFO") {
    val shiftRegLISsr = Module(
      new AdjustableShiftRegisterStream(
        params.proto,
        params.LISsize,
        sendCnt = true,
        adjust = params.rtcSize,
        enInitStore = params.enInitStore
      )
    )
...
object LISsrApp extends App {
  ...
  (new ChiselStage)
    .execute(Array("--target-dir", "verilog/LISsr"), Seq(ChiselGeneratorAnnotation(() => new LinearSorterSR(params))))
}
