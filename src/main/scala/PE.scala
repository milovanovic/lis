...
class PE [T <: Data: Real] (val params: LISParams[T], index: Int) extends Module {
  ...
  val load = Mux(io.passthrough.getOrElse(false.B), true.B && io.enableSort, (io.enableSort && ctrlLogic.io.load)) //ok!
  ...
  .elsewhen (io.active.getOrElse(true.B) && (index != 0).B) { //try to shift (index!=0) inside getOrElse
    if (params.passthrough == true)
      cellData := Mux(io.passthrough.get, io.leftNBR.data, Mux(ctrlLogic.io.leftRightShift, io.rightNBR.data, io.leftNBR.data))
    else 
  ...
  val saveCellData = RegInit(rstProto)
  ...
  val (cntLife, discard) = LifeCounter(io.enableSort && ~io.passthrough.getOrElse(false.B), ctrlLogic.io.rstPEregs, index.U, load, params.LISsize, rightOrLeftCNT, io.lisSize.getOrElse(params.LISsize.U)) // do not count if passthrough mode is active
  ...
  if (index == 0) {
    ...
    // check this
    if (params.passthrough == true)
      io.leftOutData := Mux(io.passthrough.get, io.inData, Mux(compRes, saveCellData, io.inData))
    else
      io.leftOutData := Mux(compRes, saveCellData, io.inData) // if we replace this to be io.in.data when passthrough mode is active
  }

  when (io.active.getOrElse(!(index == (params.LISsize-1)).B)) {
    //io.rightOutData := Mux(compRes, io.inData, saveCellData)
    if (params.passthrough == true) 
      io.rightOutData := Mux(io.passthrough.get, saveCellData, Mux(compRes, io.inData, saveCellData))
    else
      io.rightOutData := Mux(compRes, io.inData, saveCellData)
  }
  .otherwise {
