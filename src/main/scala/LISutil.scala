...
// available also inside sdf-fft repository
...
// consider moving it to utils repository
object AdjustableShiftRegister {

  private def applyReccursive[T <: Data](
    in:         T,
    n:          Int,
    activeRegs: Vec[Bool],
    out:        Vec[T],
...
class AdjustableShiftRegisterStream[T <: Data](
  val proto:       T,
  val maxDepth:    Int,
  val parallelOut: Boolean = false,
  val sendCnt:     Boolean = false,
  val enInitStore: Boolean = true,
  adjust:          Boolean = true)
    extends Module { //Stream
  ...
  val (adjShiftReg, adjShiftRegOut) =
    AdjustableShiftRegister.returnOut(io.in.bits, maxDepth, io.depth, resetData, en, adjust)
