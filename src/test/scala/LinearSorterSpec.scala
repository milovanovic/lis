   ...
   // for (sorterType <- Seq("LIS_FIFO")) {
      for (sorterSize <- Seq(8, 24, 64)) {
        for(sortDir <- Seq(true, false)) {
          it should f"work for UInt, sorter type $sorterType, sorter size = $sorterSize and parameter sortDir = $sortDir" in {//in {//in {
    ...
    for (sorterType <- Seq("LIS_fixed", "LIS_FIFO", "LIS_input")) {
      for (sorterSize <- Seq(2, 24, 32)) {
        for (sortDir <- Seq(true, false)) {
          it should f"work for DspReal, sorter type $sorterType and  sorter size = $sorterSize and sortDir = $sortDir" in {//in {
...
//         for (sortDir <- Seq(true,false)) {
//           it should f"work for FixedPoint, sorter type $sorterType, sorter size = $sorterSize, sortDir = $sortDir and included flushData parameter" in {
//             val params: LISParams[FixedPoint] = LISParams(
//               proto = FixedPoint(16.W, 8.BP),
//               LISsize = sorterSize,
//               LIStype = sorterType,
