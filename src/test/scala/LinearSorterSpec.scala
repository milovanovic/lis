...
class LinearSorterCNTsSpec extends FlatSpec with Matchers {
  ...
  for (sorterSubType <- Seq("LIS_fixed", "LIS_FIFO", "LIS_input")) {
   // for (sorterSubType <- Seq("LIS_FIFO")) {
      for (sorterSize <- Seq(8, 16, 64)) {
        for(sortDir <- Seq(true, false)) {
          it should f"work for UInt, sorter type $sorterSubType, sorter size = $sorterSize and parameter sortDir = $sortDir" ignore {//ignore {//in {
    ...
    for (sorterSubType <- Seq("LIS_fixed", "LIS_FIFO", "LIS_input")) {
      for (sorterSize <- Seq(2, 16, 32)) {
        for (sortDir <- Seq(true, false)) {
          it should f"work for SInt, sorter type $sorterSubType, sorter size = $sorterSize and sortDir = $sortDir" ignore {
    ...
    for (sorterSubType <- Seq("LIS_fixed", "LIS_FIFO", "LIS_input")) {
      for (sorterSize <- Seq(2, 16, 32)) {
        for (sortDir <- Seq(true,false)) {
          it should f"work for FixedPoint, sorter type $sorterSubType, sorter size = $sorterSize and sortDir = $sortDir" ignore {//in {
    ...
    for (sorterSubType <- Seq("LIS_fixed", "LIS_FIFO", "LIS_input")) {
      for (sorterSize <- Seq(2, 16, 32)) {
        for (sortDir <- Seq(true, false)) {
          it should f"work for DspReal, sorter type $sorterSubType and  sorter size = $sorterSize and sortDir = $sortDir" ignore {//ignore {
   ...
   for (sorterSubType <- Seq("LIS_fixed", "LIS_FIFO", "LIS_input")) {
      for (sorterSize <- Seq(8, 16, 32)) {
        it should f"work for FixedPoint, sorter type $sorterSubType, compile sorter size = $sorterSize and rtcSize configurable sorter size" ignore {//in {
          val params: LISParams[FixedPoint] = LISParams(
            proto = FixedPoint(16.W, 8.BP),
            LISsize = sorterSize,
            LISsubType = sorterSubType,
   ...
   //for (sorterSubType <- Seq("LIS_FIFO")) {
      for (sorterSize <- Seq(4, 15, 32)) { // test power of 2 and non power of 2 sorter size
        it should f"work for FixedPoint, sorter type $sorterSubType, compile sorter size = $sorterSize and rtcSize configurable sorting direction and sorter size" ignore {
          val params: LISParams[FixedPoint] = LISParams(
            proto = FixedPoint(16.W, 8.BP),
            LISsize = sorterSize,
            LISsubType = sorterSubType,
...
//       for (sorterSize <- Seq(2,16,32)) {
//         for (sortDir <- Seq(true,false)) {
//           it should f"work for FixedPoint, sorter type $sorterSubType, sorter size = $sorterSize, sortDir = $sortDir and included flushData parameter" ignore {
//             val params: LISParams[FixedPoint] = LISParams(
//               proto = FixedPoint(16.W, 8.BP),
//               LISsize = sorterSize,
//               LISsubType = sorterSubType,
...
//     }
    ...
    for (sorterSubType <- Seq("LIS_FIFO", "LIS_fixed", "LIS_input")) {
      for (sorterSize <- Seq(2, 4, 16, 25)) {
        for (sortDir <- Seq(true, false)) {
          it should f"work for FixedPoint, sorter type $sorterSubType, sorter size = $sorterSize, sortDir = $sortDir and with lastIn/ lastOut" in {
            val params: LISParams[FixedPoint] = LISParams(
              proto = FixedPoint(16.W, 8.BP),
              LISsize = sorterSize,
              LISsubType = sorterSubType,
