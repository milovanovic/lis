...
//TODO: Make one tester but with many functions

class LinearSorterTesterPasstrough[T <: Data](dut: LinearSorter[T], in: Seq[Double], tol: Int) extends DspTester(dut) {
  require(dut.params.passthrough == true, "This test example requires that simple passthrough functionality is included")
  poke(dut.io.out.ready, 0)
  poke(dut.io.in.valid, 0)
  
  step(5)
  
  poke(dut.io.passthrough.get, 1)
  
  val input = in.iterator
        ...
        if (peek(dut.io.out.valid)) {
          ...
          if (dut.params.LIStype == "LIS_FIFO") {
           ...
           // println(expOut.map(_.toString()).mkString(", "))
            ...
            expect_sorted_seq(dut.io.sortedData, expOut, tolLSB)
            expOut(discardPos) = tmpDataIn //peek(dut.io.in.bits)
